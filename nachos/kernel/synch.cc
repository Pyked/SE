...
#ifdef ETUDIANTS_TP
void Semaphore::P() {
    IntStatus oldLevel = interrupt->SetLevel(IntOff); // Désactiver les interruptions
    if (counter <= 0) {
    ...
    counter--;
    (void) interrupt->SetLevel(oldLevel); // Restaurer l'état des interruptions
}
#endif
//----------------------------------------------------------------------
// Semaphore::V
...
void Semaphore::V() {
    ...
    if (!waiting_queue->IsEmpty()) {
        ...
        scheduler->ReadyToRun(threadToWakeUp); // Mettre le thread de retour dans la file des prêts
    }
    (void) interrupt->SetLevel(oldLevel); // Restaurer l'état des interruptions
}
#endif
